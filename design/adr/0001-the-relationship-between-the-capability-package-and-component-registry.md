# ADR-0001: The relationship between capability packages and Component Registry
...
Do Skills/Rules/Specs incorporate the unified instance metadata of §10.6 Component Registry? Or is it only managed within the tool registry? Need to be aligned with `ai-platform-api`.
...
This ADR solidifies the "relationship between capability packages and Component Registry" into an architectural decision record and incorporates it into `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §14 open issue and is still open.
